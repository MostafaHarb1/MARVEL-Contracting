...
function renderProjectsSection(root) {
  if (!ui.selectedProjectId) {
    ...
    root.innerHTML = `
      ...
      <section class="section-card">
        <div class="row">
          ...
          <button class="btn btn-primary" id="toggle-project-form">${ui.showProjectForm ? "إخفاء" : "إضافة مشروع"}</button>
        </div>
        ${state.projects.length ? `<div class="project-grid">${cards}</div>` : '<div class="empty">لا توجد مشاريع حالياً</div>'}
      </section>
      ${ui.showProjectForm ? projectCreateFormTemplate() : ""}
    `;
    ...
    const toggle = document.getElementById("toggle-project-form");
    if (toggle) {
      toggle.addEventListener("click", () => {
        ui.showProjectForm = !ui.showProjectForm;
        if (!ui.showProjectForm) {
...
function renderProjectTabContent(project) {
  if (ui.projectTab === "boq") {
    const rows = project.boq
      .map((item) => {
        ...
        const remaining = Math.max(selfAvailable - executed, 0);
      ...
      <section class="section-card">
        ...
        ${project.boq.length ? `
          <div class="table-wrap">
            <table>
              <thead>
                <tr><th>اسم البند</th><th>الكمية الإجمالية</th><th>كمية الباطن</th><th>تشغيل ذاتي متاح</th><th>المنفذ</th><th>المتبقي الذاتي</th><th>نسبة الإكمال</th></tr>
              </thead>
              <tbody>${rows}</tbody>
            </table>
  ...
  if (ui.projectTab === "subcontract") {
    ...
    return `
      <section class="section-card">
        <div class="row"><h3 class="card-title">مقاولو الباطن</h3></div>
...
function renderExecutionSection(root) {
  ...
  const rows = flatBoq
    .map(({ project, item, progress }) => {
      ...
      <tr>
        ...
        <td>${Math.max(selfAvailable - Number(item.executedQty || 0), 0).toFixed(2)}</td>
        <td>${progressBar(progress)}</td>
      </tr>
    `;
    })
  ...
  root.innerHTML = `
    ...
    <section class="section-card">
      ...
      ${flatBoq.length ? `<div class="table-wrap"><table><thead><tr><th>المشروع</th><th>البند</th><th>الإجمالي</th><th>المسند للباطن</th><th>التشغيل الذاتي المتاح</th><th>المنفذ</th><th>المتبقي الذاتي</th><th>نسبة الإكمال</th></tr></thead><tbody>${rows}</tbody></table></div>` : '<div class="empty">لا توجد بنود متاحة</div>'}
    </section>
  `;
  ...
  projectSelect.addEventListener("change", () => {
    ...
    boqSelect.innerHTML = `<option value="">اختر المقايسة</option>${(project?.boq || []).map((b) => `<option value="${b.id}">${escapeHtml(b.itemName)}</option>`).join("")}`;
  });
  ...
  document.getElementById("add-execution-form").addEventListener("submit", (e) => {
    ...
    const selfRemaining = Math.max(selfAvailable - Number(boq.executedQty || 0), 0);
    if (executedQty > selfRemaining) {
      alert(`الكمية المنفذة تتجاوز المتبقي الذاتي (${selfRemaining.toFixed(2)})`);
      return;
    }
...
function renderSettingsSection(root) {
  ...
  const workersRows = state.workers
    .map(
      ...
      <tr>
        ...
        <td><button class="btn btn-danger" data-delete-worker="${w.id}">حذف</button></td>
      </tr>
    `,
    )
    .join("");
  ...
  if (systemUserForm) {
    systemUserForm.addEventListener("submit", (e) => {
      ...
      const phone = String(form.get("phone") || "").trim();
      if (!isKuwaitiPhone(phone)) {
        alert("رقم الهاتف الكويتي غير صحيح");
        return;
      }
...
function renderAccountsSection(root) {
  ...
  root.innerHTML = `
    <section class="section-card">
      <div class="row">
        ...
        <button class="btn btn-secondary eye-toggle" id="toggle-finance-visibility">${ui.financeVisible ? "إخفاء الأرقام" : "إظهار الأرقام"}</button>
      </div>
      <div class="kpi-grid">
  ...
  document.getElementById("toggle-finance-visibility").addEventListener("click", () => {
    ui.financeVisible = !ui.financeVisible;
    render();
  });
}
...
function bindProjectCreateForm() {
  ...
  document.getElementById("add-boq-row").addEventListener("click", () => {
    ...
    if (!itemName || qty <= 0 || !unit || unitPrice < 0) {
      alert("الرجاء إدخال بيانات بند صحيحة");
      return;
    }
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!name || !startDate) {
      alert("اسم المشروع وتاريخ البداية مطلوبان");
      return;
    }

    if (!projectDraft.boq.length) {
      alert("يجب إضافة بند مقايسة واحد على الأقل");
      return;
    }
...
function bindSubcontractForm(project) {
  ...
  const refresh = () => {
    ...
    const availableForSubcontract = Math.max(Number(boq.qty || 0) - assigned - Number(boq.executedQty || 0), 0);
    hint.textContent = `إجمالي البند: ${num(boq.qty)} | منفذ ذاتياً: ${num(boq.executedQty || 0)} | مسند للباطن: ${num(assigned)} | المتاح للباطن: ${num(availableForSubcontract)}`;
  };
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (qty > availableForSubcontract) {
      alert(`الكمية المطلوبة تتجاوز المتاح في البند (${availableForSubcontract.toFixed(2)})`);
      return;
    }
...
function projectCreateFormTemplate() {
  return `
    <section class="section-card">
